...
if (isset($_POST['action']) && $_POST['action'] == 'insert') {
    ...
    $telNumber = (int) ($_POST['telefonas']);
    
    $q = "INSERT INTO VARTOTOJAI (vardas, pavarde, elpastas, tipas, telefonas) values ('$name','$surName','$eMail','$type', $telNumber)";
    $db->query($q);
    echo $db->error;
}
?>
		...
		<div class="row">
			...
			<h2>
				...
				<button type="button" class="btn btn-primary" data-toggle="modal"
					data-target="#myModal">Pridėti vartotoją</button>
			</h2>
			<!-- Modal -->
						...
						<div class="modal-body">
							<form action="vartotojai.php" method="post">
								...
								<div class="form-group">
									<label for="vardas">Tipas</label> <select name="tipas"
								...
								<div class="form-group">
									<label for="vardas">Vardas</label> <input name="vardas"
										type="text" class="form-control" id="vardas">
								</div>

								<div class="form-group">
									<label for="pavarde">Pavardė</label> <input name="pavarde"
										type="text" class="form-control" id="pavardė">
								</div>
								<div class="form-group">
									<label for="pavarde">El. paštas</label> <input name="elpastas"
										type="text" class="form-control" id="elpastas">
								</div>
								<div class="form-group">
									<label for="pavarde">Tel. numeris</label> <input
										name="telefonas" type="text" class="form-control"
										id="telefonas">
								</div>
			...
			<div class="table-responsive">
				<table class="table table-striped table-sm">
					...
					<tbody>
                 <?php
                $q = 'SELECT * FROM vartotojai ';
                $result = $db->query($q);
                while ($row = $result->fetch_assoc()) {
                    ?>
                <tr>
							<td><?php echo $row['id']?></td>
							<td><?php echo $row['tipas']?></td>
							<td><?php echo $row['vardas']?></td>
							<td><?php echo $row['pavarde']?></td>
							<td><?php echo $row['elpastas']?></td>
							<td><?php echo $row['telefonas']?></td>
							<td><?php echo $row['grupes_pavadinimas']?></td>
							<td><a href="vartotojai-redaguoti.php?id=<?php echo $row['id']?>"
								class="btn btn-success">Redaguoti</a></td>
							<td><a href="vartotojai-istrinti.php?id=<?php echo $row['id']?>"
